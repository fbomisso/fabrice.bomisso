...
\* `Fact\_Ventes` : 1 000 lignes

\* `Fact\_Achats` : 1 000 lignes

\* `Fact\_Stock` : 600 lignes
...
\* `Dim\_Date` : 1 096 lignes

\* `Dim\_Magasins` : 12 lignes

\* `Dim\_Produits` : 100 lignes

\* `Dim\_Clients` : 1 000 lignes



La relation `Dim\_Clients → Dim\_Magasins` a été désactivée afin d'éviter les chemins ambigus vers `Fact\_Ventes`. Elle peut être utilisée ponctuellement avec `USERELATIONSHIP()`.
...
| CA Total            | `SUMX(Fact\_Ventes, \[quantité] \* \[prix\_unitaire])`                  | KPI principal           |
...
| Nb Clients Actifs   | `DISTINCTCOUNT(Fact\_Ventes\[id\_client])`                            | Suivi de la clientèle   |

| Produits en Rupture | `COUNTROWS(FILTER(Fact\_Stock, \[quantité] = 0))`                    | Alerte stock            |
...
│   └── Pilotage\_Décisionnel\_d'un\_Réseau\_de\_Distribution\_Multi-Magasins\_avec\_Power\_BI.pdf
